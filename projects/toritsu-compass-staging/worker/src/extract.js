...
// 部活の言い換え・略称。学校サイトの部活名は「パソコン部」「バスケットボール部」で、
// 保護者と中学生は「プログラミング」「バスケ」と言う。展開しないと LIKE で1件も
// 当たらず、データがあるのに0件と答えてしまう（2026-08-19 欠陥チェックで発覚）。
//
// ⚠️ プロトタイプ側（prototype/index.html の CLUB_ALIAS）と**同じ内容にそろえる**。
//    片方だけ増やすと、同じ言葉で聞いても画面とAPIで結果が変わる。
//
// ⚠️「園芸」「情報」は入れない。学科の言い回しとして DEPT_WORDS が先に扱う語で、
//    ここへ入れると「園芸をやりたい」が農業系の希望ではなく園芸部の指名になる。
const CLUB_ALIAS = {
  // 球技
  "バスケ": ["バスケットボール"], "バスケット": ["バスケットボール"],
  "バレー": ["バレーボール"], "バド": ["バドミントン"], "ハンド": ["ハンドボール"],
  "フットサル": ["サッカー", "フットサル"],
  // 陸上・水泳
  "マラソン": ["陸上"], "駅伝": ["陸上"],
  "スイミング": ["水泳"], "競泳": ["水泳"],
  // 音楽
  "ブラスバンド": ["吹奏楽"], "ブラバン": ["吹奏楽"], "吹部": ["吹奏楽"],
  "軽音": ["軽音楽"], "バンド": ["軽音楽"],
  "コーラス": ["合唱", "コーラス"], "太鼓": ["和太鼓"], "琴": ["箏曲"],
  // 文化
  "芝居": ["演劇"], "チア": ["チアリーディング"],
  "習字": ["書道"], "カメラ": ["写真"],
  "漫研": ["漫画研究"], "漫画": ["漫画研究"], "マンガ": ["漫画研究"],
  "生け花": ["華道"], "いけばな": ["華道"], "お茶": ["茶道"],
  "文学": ["文芸"], "天体": ["天文"],
  "料理": ["クッキング", "家庭科"], "調理": ["クッキング", "家庭科"],
  "百人一首": ["かるた"],
